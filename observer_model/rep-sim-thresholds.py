...
# delta to scale the decision threshold with period = 0.1

# after 02.01.2026 changes
# period_min = 10
# period_max = 30
# period_domain = np.logspace(np.log10(period_min), np.log10(period_max), num=5, dtype=int)
# delta to scale the decision threshold with period = 0.12
...
do_repeat_domain = [True]  # you can add False if needed
n_minus_k_norm = False  # normalize the correlation by (n-k) with statmodels
internal_noise_level = 0.2

# Decision threshold baseline and slope for increase with period
...
for n in range(num_simulations):
    ...
    # Iterate over experimental parameters
    for period, repetitions, snr, do_repeat, base_threshold in it.product(
        period_domain, repetitions_domain, snr_domain, do_repeat_domain, decision_domain):

        # # Compute log-scaled decision threshold based on period
        # LOGARITHMIC SCALING from period_min to period_max → [0, 1]
        raw_scaling = np.log(period / period_min)  # log(1) = 0 at period = period_min
        max_raw_scaling = np.log(period_max / period_min)
        normalized_scaling = raw_scaling / max_raw_scaling  # scales to [0, 1]

        # Scale to decision threshold range [base_threshold, base_threshold + 0.3]
        ...
        lags_nonneg = lags[zero_lag_index:]  # positive lags only
        ...
        if peak_idx.size == 0:
            ...
            indices = np.arange(1, len(corr))
            data_with_index = list(zip(corr[1:], indices))
        ...
        decision_threshold.append(decision_thresh)
...
df_wide.to_csv("process_model_simulations_threshold_change_new_params2.csv", index=False)



# %%
df_wide = pd.read_csv("process_model_simulations_threshold_change_new_params2.csv")    
