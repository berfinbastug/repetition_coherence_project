...
# search_space_period = np.arange(period_min-2, period_max+2 + 1)
...
# repetitions_domain = np.linspace(2, 50, num=9, dtype=int)  # min = 2
...
for n in range(0, num_simulations):
    ...
    for period, repetitions, snr, do_repeat, decision_thresh in it.product(  # experiment factors
        ...
        # Scale noise amplitude based on period
        ...
        # Set final scaling level
        scaling_level = internal_noise_level + (normalized_scaling * 0.19)
        ...
        if plot_while_simulating:
            ...
            fig, (ax_stim, ax_corr) = plt.subplots(2, 1, figsize=(8, 5), layout="constrained")
            fig.suptitle(f"Per: {period.item()} Reps: {repetitions.item()} "
                         f"SNR: {snr.item()} Estim: {estimation} Error: {error}")

            ax_stim.plot(noised_stim)
            ax_stim.set_title('stimulus')
            ax_stim.set_xlabel('Sample Number (time)')
            ax_stim.set_ylim(- np.abs(noised_stim).max(), np.abs(noised_stim).max())
            ax_stim.vlines(period, 
                           ymin= noised_stim.min(), ymax=noised_stim.max(), 
                           color="green", linewidth=2, alpha=0.6)
            
            ax_corr.plot(lags_nonneg, corr)
            # ax_corr.set_title('Autocorrelation function')
            ax_corr.set_xlabel('Lag')
            ax_corr.set_ylabel('Correlation')
            ax_corr.vlines(estimation, 
                           ymin= corr.min(), ymax=corr.max(), 
...
# save the dataframe
df_wide.to_csv("process_model_simulations_internal_noise_new_params2.csv", index=False)

# %%
# read the data frame
df_wide = pd.read_csv("process_model_simulations_internal_noise_new_params2.csv")
...
fig.subplots_adjust(wspace=0.3, right=0.88, bottom=0.25)
...
for i, period in enumerate(selected_periods):
    ...
    # Track maximum for this subplot
    max_autocorr = -np.inf
    max_x = None
    ...
    for snr in snrs:
        ...
        if df_plot["Autocorr"].max() > max_autocorr:
            max_autocorr = df_plot["Autocorr"].max()
            max_x = df_plot.loc[df_plot["Autocorr"].idxmax(), "Reps"]

    # Add horizontal dashed line at the max value
    ax.axhline(y=max_autocorr, color='gray', linestyle='--', linewidth=9, alpha=0.7)
