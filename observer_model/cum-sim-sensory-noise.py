...
for trial in range(num_trials):
    ...
    for period, snr, base_threshold in it.product(period_domain, snr_domain, decision_domain):
        ...
        for reps in range(1, max_reps + 1):
            ...
            int_noise = np.random.uniform(-1.0, 1.0, size=stim.shape)
            ...
            # Add to stimulus
            noised_stim = stim + scaled_noise
            noised_stim -= noised_stim.mean()
            noised_stim /= rms(noised_stim)
            # Compute autocorrelation
            corr, confint = acf(noised_stim, adjusted=False, alpha=0.05, nlags=noised_stim.shape[0]-1)
...
# save the data frame
df_detect.to_csv('ncycle_sim_results_int_noise_new_params.csv', index=False)

# %%
# read the data frame from the saved csv file
df_detect = pd.read_csv('ncycle_sim_results_int_noise_new_params.csv')
