...
# %%
# ========== READ DATA ==========
df_detect = pd.read_csv(f'simulation_results/ncycle_sim_results_scale_independent.csv')
df_wide = pd.read_csv(f'simulation_results/acf_model_simulations_scale_independent_seed{RANDOM_SEED}.csv')
...
fig2.savefig('plot2_scale_independent.svg', dpi=300, transparent=True, bbox_inches='tight', pad_inches=0)
plt.show()
...
# ==========================================================================
fig1, axes = plt.subplots(1, 2, figsize=(2.8, 1.3))
...
# crit_dense = base_val + norm_dense * slope

LINEWIDTH = 1.3
MARKER_SIZE_DECISION = 6
MARKER_SIZE = 2
# ax.plot(periods_dense, crit_dense, color='gray', linewidth=LINEWIDTH, alpha=0.7)
# for period in period_domain:
#     norm_scale = np.log(period / period_min) / np.log(period_max / period_min)
#     crit = base_val + norm_scale * slope
#     ax.plot(period, crit, 'o', color=period_colors[period], markersize = MARKER_SIZE_DECISION,
#             markeredgecolor='black', markeredgewidth=1, zorder=3)
#     # ax.annotate(period_labels[period], xy=(period, crit),
#     #             xytext=(10, -5), textcoords='offset points',
#     #             fontsize=20, fontweight='bold', color=period_colors[period])

# # ax.set_xlabel('Unit Duration', fontsize=30, fontweight='bold', labelpad=10)
# # ax.set_ylabel('Decision\nCriterion', fontsize=30, fontweight='bold', labelpad=10)
# ax.set_xticks([])
# ax.set_yticks([])
# ax.spines['top'].set_visible(False)
# ax.spines['right'].set_visible(False)
...
fig1.savefig('plot1_scale_independent.svg', transparent=True, bbox_inches='tight', pad_inches=0)
plt.show()
