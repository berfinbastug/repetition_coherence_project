...
PERC_VALS = [0, 0.222, 0.444, 0.667, 1]
FIGSIZE = (3.5, 1.2) 
...
#
# =====================
fig, axes = plt.subplots(1, 5, figsize=(7, 1.4), sharex=True)
...
for i in range(len(PERC_VALS)):
    ...
    for time in times_extended:
        ...
        if is_important_time:
            ax.axvline(x=time, color='gray', linestyle='--', linewidth=2, alpha=0.7)
        else:
            ax.axvline(x=time, color='gray', linestyle='--', linewidth=0.3, alpha=0.3)

    # Add horizontal reference lines for frequency
    for freq in freqgrid_plot:
        ax.axhline(y=freq, color='gray', linestyle='--', linewidth=0.3, alpha=0.3)
    ...
    for rep in range(NREP):
        for i, j in zip(r_new, c_new):
            ...
            end_time = start_time + 0.05
            freq = bigzf[i, j + rep * ntsteps]
            ax.plot([start_time, end_time], [freq, freq], color="#8A8A8AB6", linewidth=1.5)
...
# fig.savefig('coherence.svg', dpi=300, transparent=True, bbox_inches='tight', pad_inches=0)
plt.show()